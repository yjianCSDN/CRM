    ...
    >
      ...
      >
        ...
        <el-form-item label="服务处理">
          <el-input v-model="updateServeInfo.serviceProce" style="width: 70%;" type="textarea"
                    :autosize="{ minRows: 2, maxRows: 4 }" placeholder="请输入服务处理过程"/>
        </el-form-item>
...
export default {
  ...
  data() {
    let customerServeQuery = reactive({page: 1, limit: 10, customer: "", serveType: "", state: "fw_003", assigner: ""})
    let serveList = reactive([])
    let total = ref("")
    let processingVisible = ref(false)
    let updateServeInfo = reactive({
      serveType: "", customer: "", overview: "", assignTime: "",
  ...
  methods: {
    ...
    assign() {
      if (this.updateServeInfo.serviceProce !== "") {
        ...
        ElMessage({type: "info", message: "请填写相应字段"})
      }
